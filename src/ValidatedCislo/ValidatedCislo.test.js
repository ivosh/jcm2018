...
it('dva startující na trase', () => {
  ...
  const component = renderer.create(<ValidatedCislo startujici={startujici} onCisloSubmitted={fakeOnCisloSubmitted} />);
  expect(component.toJSON()).toMatchSnapshot();
});
...
it('jeden startující na trase a jeden dokončil', () => {
  ...
  const wrapper = mount(<ValidatedCislo startujici={startujici} onCisloSubmitted={fakeOnCisloSubmitted} />);
